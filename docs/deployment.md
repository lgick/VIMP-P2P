...
2. Создайте (или отредактируйте) переменную `SERVERS_MATRIX`:
   ...
   _(`domain` и `port` должны строго совпадать с указанными на Шаге 3; значения полей соответствуют переменным `VIMP_*` — см. [configuration.md](configuration.md#переменные-окружения-env))._
