...
Просто `git push` в ветку `main` — GitHub Actions автоматически обновит все серверы из `SERVERS_MATRIX`. Клиентская статика и WASM-ядро внутри образа; уже открытые комнаты доиграют на старом коде хоста (обновление кода комнат на лету — Этап 5.2 плана).
