...
> [deployment.md](deployment.md#central-auth-сервис-packagesauth)).
...
`nick` уникален на весь сервис (один ник на все игры). Единственный модуль,
трогающий эти таблицы, — `packages/auth/src/UserRepository.js`.

## REST API

| Эндпоинт | Назначение |
| --- | --- |
| `GET /oauth/:provider/start?returnUrl=` | редирект на страницу провайдера; `returnUrl` и CSRF-nonce упакованы в подписанный stateless `state` (`src/lib/oauthState.js` — HMAC, без серверной сессии) |
| `GET /oauth/:provider/callback` | обменивает `code`, находит/создаёт пользователя по `(provider, providerUid)`, редиректит на `returnUrl` с `?token=` (ник уже есть — полноценный identity JWT) либо `?pendingToken=` (первый вход, ник не выбран) |
| `POST /nick` (Bearer pending-токен, `{ nick }`) | проверяет ник по `NAME_REGEXP` и глобальной уникальности, сохраняет, возвращает `{ token }` (полный identity JWT). `409 { error: 'nickTaken' }` при гонке |
...
подписан RS256, короткоживущий (`config.jwt.expiresIn`, 15 мин по
умолчанию), проверяется с `issuer: 'vimp-auth'`. Pending-токен (выдаётся
между OAuth-колбэком и `POST /nick`) вместо этого несёт `pending: true` и не
...
4. **Выбор ника**: отправка формы ника делает `POST {authServiceUrl}/nick`
   ...
   проксируется мастером. При успехе полученный identity-токен сохраняется
   и открывается лобби; `409 nickTaken` / `400 invalidNick` выводятся
   inline.
...
`tests/auth/` (node-проект Vitest): `validators.test.js`, `jwt.test.js`
(подписывает одноразовой RSA-парой, мокает `config/auth.js`),
`github.test.js` (мокает `fetch`), `oauthState.test.js`,
`UserRepository.test.js` (заглушка `{ query() }` — для юнит-тестов реальный
PostgreSQL не нужен).
...
auth-сервиса, накопление rank, flush/flushAll), плюс покрытие rank/flush,
добавленное в `tests/host/RoundManager.test.js`, и проброс токена в
