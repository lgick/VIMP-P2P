...
1. **Опубликовать `@vimp/engine`.** Игра импортирует его только через публичный `exports` ([packages/engine/package.json](../../packages/engine/package.json): `./lib/*`, `./config/*`), у которого уже есть `files`-allowlist (только `src/lib`, `src/config` — `src/master/*` специфичен для мастер-сервера и в пакет не попадает) и `publishConfig`, проверенные через `npm pack --dry-run`. Осталось: реально опубликовать в реестр (npm, GitHub Packages) либо подключить как Git-зависимость с тегом, и зафиксировать версию вместо текущего workspace-диапазона `"*"` в [games/tanks/package.json](../../games/tanks/package.json).
2. **Опубликовать `vimp-engine-core`.** Сейчас `games/tanks/core/Cargo.toml` подключает его как относительную `path`-зависимость (`../../../packages/engine/core`); крейт уже собирается автономно в пакет (`cargo package -p vimp-engine-core` сам резолвит `workspace = true`-зависимости в конкретные версии). Осталось: перевести зависимость игры на `git` с тегом/rev (crates.io — только если движок задуман как публично переиспользуемый вне этого проекта).
