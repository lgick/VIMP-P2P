...
> **Rust-тулчейн в сборке.** WASM-ядро игры (`games/tanks/core/`) — cdylib, собираемый `wasm-pack`; движковая половина (`packages/engine/core/`, обычный rlib) подтягивается как path-зависимость — [Dockerfile](../../Dockerfile) собирает его отдельной стадией `core-builder` (`rust:slim` + `wasm-pack build games/tanks/core`). Node-стадия затем выполняет `npm run game:build` (собирает бандл плагина `@vimp/tanks` — client/host-точки входа, WASM-ассет, карты, звуки, `manifest.json` — в `games/tanks/dist/`), а следом `npm run build:app` (аудио + Vite-сборка движка) с уже готовым `pkg-web`. Runner-стадия копирует и `packages/engine/dist/`, и `games/tanks/dist/`; мастер читает плагин только через `GameCatalog` (`dist/manifest.json` + `dist/maps/*.json`) — исходный код игры он никогда не импортирует, поэтому `games/tanks/src/` в образ не попадает.
...
[← Предыдущая: Расширение игры](extending.md) · [Следующая: Plugin API →](plugin-api.md)
