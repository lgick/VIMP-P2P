...
`HostSession`: `hostId` (uuid), `name`, `maxPlayers` (clamp к `host.maxPlayersLimit`, целевой размер комнаты — 8), `currentPlayers`, `mapName`, `region`, `ip`, `gameId`/`gameVersion` (какую игру-плагин и версию манифеста объявил хост в `register_host`; `null`, пока хосты его не присылают — Этап 6.4), `status` (`online`/`banned`), `reportCount` + `reporters` (`Map` репортёр → timestamp: уникальность жалоб и окно давности), `reportReasons` (причины жалоб, аудит — наружу не отдаются, capped), `lastSeen`.
...
фильтр по игре в лобби (Этап 6.3+); пока `null` для хостов, ещё не
объявляющих его в `register_host` (Этап 6.4).
...
Этот одноигровой маршрут старше `GameCatalog` (Этап 6.2) и оставлен как
совместимость: клиентский поток join/«создать сервер»/загрузки
`ClientPlugin` перешёл на per-game маршруты ниже в Этапе 6.3, но роль хоста
всё ещё берёт каталог карт своей комнаты с этого легаси-маршрута — эта часть
`connectAsHost`/`host.worker.js` переедет на `/games/:id/maps/*` в Этапе 6.4
(динамическая загрузка `HostPlugin`).
...
- `GET /games/:id/maps/manifest.json` / `GET /games/:id/maps/:name` — та же
  форма, что у старых маршрутов `/maps/*` выше, но per-game (строится из
  `games/<id>/dist/maps/*.json`).
