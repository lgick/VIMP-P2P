...
`games/tanks/src/config/auth.js`). После этой проверки Worker сам —
...
- **Загрузка на join**: `HostGame.createUser()` запускает
  ...
  `data.playerState?.defaultState`, например
  `games/tanks/src/config/game.js`) — недоступность auth-сервиса никогда не
  блокирует вход.
...
- **Синхронизация обратно**: `flush(participantId)` шлёт `PUT` текущих
  rank+state участника на мастер (`Promise.allSettled`, best-effort —
  ошибки проглатываются, следующий flush повторит попытку с уже
  накопленными данными); `flushAll()` сливает всех текущих участников. Две
  точки жизненного цикла вызывают `flushAll()`: `RoundManager.createMap()`
  (смена карты) и `RoundManager._checkTeamWipe()` (конец раунда) — обе
  рядом с уже существующими вызовами `Stat.reset()`/`Stat.updateHead()` на
  тех же границах. `HostGame.removeUser()` делает ещё один best-effort
  `flush()` для выходящего участника перед удалением его записи в
  `PlayerDataSync`.
