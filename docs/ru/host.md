...
`gameConfig`, `authSchema`, `chatCommands` (`/bot`), `systemMessages`
(группа `b:*`), `createModules` (scripted-модуль ботов),
`buildClientGameConfig()` (игровая половина CONFIG_DATA); опционально
`onCoreEvent` для игровых `custom`-событий ядра (танки его не задают). Его
потребляют `host.worker.js` (конфиги/авторизация) и `HostGame` (команды,
коды, модули, `onCoreEvent`).
