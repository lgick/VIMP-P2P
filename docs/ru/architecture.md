...
games/tanks/     — @vimp/tanks: игра (npm workspace)
  ...
  core/          — vimp-tanks-core (Rust → WASM, pkg-web/pkg-node): танки,
                   оружие, боты, предикт, спавн снарядов (docs/core.md)
...
└─ Web Worker (host.worker.js)   — авторитетная симуляция ~120 Гц
    ├─ GameCore (WASM, games/tanks/core/) — физика, оружие, боты
...
- **Предсказание** (`games/tanks/core/src/client/predictor.rs`): свой танк симулируется репликой авторитетной модели движения (формулы общие с ядром — `motion.rs`); хост подтверждает ввод (`lastInputSeq`), reconciliation переигрывает неподтверждённые вводы, расхождение плавно затухает.
- **Клиентский спавн снарядов** (`games/tanks/core/src/client/shot.rs`): выстрел виден и слышен мгновенно, дубли от хоста подавляются по id автора.
...
Полная разметка домиграционного дерева. Миграция завершена: перечисленные
MIXED-файлы разрезаны, как описано (что именно вырезано — в скобках);
таблица сохранена как летопись «что куда уехало».

| Область | ENGINE | GAME | MIXED (что вырезано) |
| --- | --- | --- | --- |
...
- **Паритет реплики движения**: авторитетное движение (Rapier) и реплика клиентского предикта делят формулы тика (`games/tanks/core/src/motion.rs`); паритет интеграции закреплён cargo-тестами (`client::predictor::parity`) — любая правка движения в ядре или коэффициентов `models.js` требует прогона `npm run core:test`.
