...
Мастер собирает свой конфиг в единое хранилище `src/lib/config.js` (доступ по пути с двоеточием) в [src/master/main.js](../../src/master/main.js); Worker хоста ([src/host/host.worker.js](../../src/host/host.worker.js)) собирает конфиг игры как merge движковых дефолтов (`hostDefaults`) и игровой половины из `HostPlugin` (`@vimp/tanks/host/index.js`: `gameConfig`, `authSchema`, `buildClientGameConfig()`), применяя поверх настройки комнаты. Клиент получает свой конфиг (CONFIG_DATA) от хоста при подключении (порт `0`).
...
- **`panel`** — контейнер `containerId` (движок); сопоставление серверных ключей (`t`, `h`, `wa`, `w1`, `w2`) полям (`keys`) и схема ячеек `elems` (игра) — DOM панели генерирует `PanelView` по схеме.
...
Конфиг авторизации игры ([games/tanks/src/config/auth.js](../../games/tanks/src/config/auth.js)): id DOM-элементов (`elems`), параметры формы (`params`) и игровые валидаторы (`validators`). Каждый параметр: `name`, значение по умолчанию, `validator` (имя функции) и ключ `storage` для localStorage. Движковый валидатор — `isValidName` ([src/lib/validators.js](../../src/lib/validators.js)); игровые (например `isValidModel` — модель есть в `models.js`) инжектируются в `validateAuth` третьим аргументом. Валидация выполняется и на клиенте (валидаторы из бандла игры), и повторно хостом (Worker); по проводу (`AUTH_DATA`, порт 1) уходят только `elems`/`params` — код валидаторов не передаётся. В этапе 6 конфиг приедет через `HostPlugin.authSchema`.
