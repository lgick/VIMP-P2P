...
- `init(room, handoff?)` — применяет настройки комнаты к конфигу игры
  ...
  инициализирует ядро, создаёт `HostGame`, отвечает `ready`; `handoff` —
  состояние эстафеты Worker'ов (Этап 5.2): комната восстанавливается вместо
  холодного старта. Сбой (WASM/конфиг/handoff-мета) — сообщение
...
- `prepare_handoff` / `resume` / `handoff_complete` — протокол эстафеты
  Worker'ов (Этап 5.2, см. одноимённый раздел ниже).
...
- **эстафета Worker'ов** (Этап 5.2): `requestHandoff(cb)` (остановка игры и
...
- **`HostController`** — спавнит Worker (по `workerUrl` из манифеста мастера,
  Этап 5.2; без него — бандловый `new Worker(new URL('host.worker.js'),
...
**Эстафета Worker'ов (Этап 5.2)** проверяется только на собранном `dist`
