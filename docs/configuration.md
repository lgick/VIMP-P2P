...
> ⚠️ Коэффициенты `models.js` используются и ядром, и клиентской репликой движения (`TankPredictor`). Их изменение проверяется паритет-тестом `tests/core/predictorParity.test.js`.
