...
**Жалоба `/ban`** (Этап 5.3) идёт **вне игрового протокола портов**: клиент перехватывает команду до отправки хосту и шлёт `report_host { hostId, reason }` по сигнальному WS мастера (`SignalingClient.reportHost`), минуя P2P-канал с хостом. Причина — в том, что хост исполняет `CommandProcessor` у себя и мог бы отфильтровать жалобу на самого себя. Логика бана — на мастере ([master.md](master.md#соц-модерация-ban-этап-53)).
