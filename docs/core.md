...
| Метод | Исторический аналог (легаси-сервер) | Назначение |
| --- | --- | --- |
...
## Известные отличия от легаси JS-версии (зафиксированы осознанно)

- **Rapier 0.34 (нативный) вместо compat 0.19**: траектории совпадали с
  легаси-сервером в допуске паритет-тестов (0.5 юнита на 120 тиков), но не
  бит-в-бит; после демонтажа легаси эталон — само ядро.
...
- `remove_tank` сам ставит null-маркер удаления в следующий кадр (на
  легаси-сервере это делала связка `removedPlayersList` + тик фасада).
