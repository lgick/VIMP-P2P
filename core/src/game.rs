...
/// `G::Sim` (см. core/src/sim.rs, core/src/tanks.rs) — движок зовёт её
/// только через `on_fixed_step`/`on_contacts`/`on_before_destroy`/
/// `on_ai_tick`, не зная деталей конкретной игры.
pub struct EngineSim<G: GameDef = TanksGame> {
    pub cfg: CoreConfig,
...
/// раньше работали с монолитным `GameState`.
pub type GameState = EngineSim<TanksGame>;
...
impl<G: GameDef> EngineSim<G> {
    ...
    pub fn spawn_actor(&mut self, game_id: u32, model_name: &str, team_id: u8, x: f32, y: f32, angle_deg: f32) -> Result<(), String> {
        self.sim
            .spawn_actor(&mut self.world, &self.cfg, &mut self.events, game_id, model_name, team_id, x, y, angle_deg)
    }
    ...
    pub fn reset_all_vitals(&mut self) {
        self.sim.reset_all_vitals(&self.cfg, &mut self.events);
    }
    ...
    pub fn apply_input(&mut self, game_id: u32, seq: u32, action: &str, key_name: &str) {
        self.sim.apply_input(&self.cfg, game_id, seq, action, key_name);
    }
    ...
    pub fn tank_alive(&self, game_id: u32) -> bool {
        self.sim.tank_alive(game_id)
    }

    pub fn tank_position_rounded(&self, game_id: u32) -> Option<[f32; 2]> {
        self.sim.tank_position_rounded(&self.world, game_id)
    }
    ...
    pub fn step(&mut self, dt: f32) {
        ...
        self.sim.refresh_cached(&self.world, &self.cfg);
    ...
    pub fn build_snapshot_blocks(&mut self) -> Vec<(String, Block)> {
        let (mut blocks, has_events) = self.sim.build_snapshot_blocks(&self.cfg);

        // динамические элементы карты — каждый отправляемый кадр
        if let Some(map) = &self.map {
            blocks.push((map.set_id.clone(), Block::Dynamics(map.dynamic_map_data(&self.world))));
        }
    ...
    pub fn remove_players_and_shots(&mut self) -> Vec<String> {
        self.sim.remove_players_and_shots(&mut self.world, &self.cfg)
    }
