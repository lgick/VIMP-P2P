...
| 4a.2 | schema-driven снапшот, `SNAPSHOT_FORMAT_VERSION → 4` | `packages/engine/src/config/opcodes.js` (44 стр.) | `ENGINE_API_VERSION`@9, `SNAPSHOT_FORMAT_VERSION`@15 (=3 → 4), `SNAPSHOT_KEYS`@20 (m1/w1/w2/w2e/c1/c2, `{id,kind}` → полная схема блока + `playerState`), `HOT_FLAGS`@39 |
| 4a.2 | generic `reconstructHot` вместо захардкоженной раскладки танка | `packages/engine/src/client/main.js` (1261 стр.) | `reconstructHot`@533 (зовётся @602) |
| 4a.3 | стандартные события `panelSet/panelActive/death/shake/custom` + generic-роутинг (снимает временный eventRouter из PR 3.1) | `packages/engine/src/host/GameCoreAdapter.js` + `games/tanks/src/host/coreEventRouter.js` | `_drainEvents`@118-124 (мост `_eventRouter` → generic-роутинг); `coreEventRouter.js` удаляется |
